...
public class YellowPlayer extends Player
{
 ...
 //quick constructor
 public YellowPlayer(ludo m)
 {
  super(m);
  setX_cordinate(96);//26//96
  setY_cordinate(238);//200//238
 }

 //constructor
 public YellowPlayer(ludo m, int button_num)
 {
  super(m);
  setColorToYellow();
  scrn_height = getScreen_height();
  setInit_button_number("Y");
  setStarting_x_cordinate((40.41666666666667/100)*screen_width);//128
  setStarting_y_cordinate((74.6/100)*scrn_height);//57
  
  //initial position of buttons
  int x_init = (int) ((11.00/ 100) * screen_width)-1;
...
//determines if Yellow has finished the game
 public static void determineIfYellowHasFinished()
 {
  if (number_of_buttons_allowed<=0)
  {
   ludo.setDrawYellowPlayerHasFinishedSign(true);
   ludo.setYellow_has_finished(true);
  }
 } 
}
